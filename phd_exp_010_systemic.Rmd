...
```{r survival-by-genotype, fig.dim=c(5, 5), fig.align='center', out.width="75%"}
...
```{r survival-by-genotype-and-replicates, fig.dim=c(6, 6), fig.align='center', out.width="75%"}
fit_replicate = survfit(Surv(Time_to_death, Censor) ~ Treatment + Age + Line + Internal_replicate,
                   data = data_expanded)

plot_replicate = ggplotprep2(fit_replicate, times = c(seq(0, 48, by = 8))) %>%
  mutate(Condition = str_remove_all(Condition,"[a-zA-Z]*=")) %>%
...
```{r survival-by-genotype-and-sex, fig.dim=c(5, 5), fig.align='center', out.width="75%"}
