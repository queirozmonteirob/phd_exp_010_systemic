...
output:
  ...
  pdf_document:
    toc: true
    toc_depth: '3'
subtitle: 'Effect of Rapamycin Treatment on Survival to Systemic Infection with *P.
  rettgeri* '
editor_options:
...
```{r survival-by-genotype, fig.dim=c(6, 6), fig.align='center', out.width="75%"}
...
```{r survival-by-genotype-and-replicates, fig.dim=c(7, 7), fig.align='center', out.width="75%"}
...
ggplot(plot_replicate, aes(x = Time, y = Survival, colour = Treatment)) +
  ...
  scale_color_manual(values = colours[c(1,7)]) +
  scale_linetype_manual(values = c("solid", "dashed")) +
  labs(title = "Effect of rapamycin treatment on survival \nto systemic P. rettgeri infection \nacross genotypes and replicates",
       caption = paste("Number of observations (n) per genotype:", plot_caption)) +
  scale_y_continuous("Proportion of survivors",
                     limits = c(0, 1),
                     breaks = c(0, 0.2, 0.4, 0.6, 0.8, 1)) +
...
```{r survival-by-genotype-and-sex, fig.dim=c(6, 6), fig.align='center', out.width="75%"}
...
# Regression and Likelihood Ratio Tests {.tabset}
...
<div class = "col-sm-6">
```{r, results='hold', fig.dim=c(4,6), out.width="80%"}
...
model1 = coxme(Surv(Time_to_death, Censor) ~ 
                         Age + Treatment + Sex + (1|Internal_replicate) + (1|Vial_ID), 
                       data = data_wdah)
summary(model1) 
...
model2 = coxme(Surv(Time_to_death, Censor) ~ 
                         Age + Treatment + Sex + Treatment*Sex + (1|Internal_replicate) + (1|Vial_ID), 
                       data = data_wdah)
summary(model2)
test1 = anova(model1, model2)
print(test1)
