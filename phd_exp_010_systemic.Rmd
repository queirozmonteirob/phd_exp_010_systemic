...
```{r, fig.dim=c(5,5), fig.align='center', out.width="50%"}
...
ggplot(forest_plot,
   ...
   coord_cartesian(ylim = c(-1, 1), expand = TRUE, clip = "on") +
   xlab("") +                                                                    
   theme_cowplot() +
...
coxme_gt %>%
  modify_header(update = c(estimate ~ "**log(HR)**",
                           label ~ "**Variable**")) %>%
  italicize_levels() %>% 
  modify_caption(caption = paste("<div style='font-weight: bold; text-align: center; color: black'>coxme:", 
